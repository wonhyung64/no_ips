...
for seed in range(10):
    ...
    wandb.run.name = f"heckman_{expt_num}"
    ...
    checkpoint = torch.load(f"./assets/selection_model/pointwise.pth")
    ps_model.load_state_dict(checkpoint["model_state_dict"])
    ps_model = model.to(device)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for idx in range(total_batch):
            ...
            rec_loss = loss_fcn(torch.nn.Sigmoid()(pred+ps_pred.detach()), sub_y)
        ...
        if epoch % evaluate_interval == 0:
            ...
            ps_pred, _, __ = ps_model(x_test_tensor)
            pred = (pred_ + ps_pred).flatten().cpu().detach().numpy()
