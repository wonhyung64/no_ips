...
for seed in range(10):
    ...
    wandb.run.name = f"ctcvr_{loss_type}_{expt_num}"
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for idx in range(total_batch):
            ...
            ctcvr_loss = loss_fcn(nn.Sigmoid()(ps_pred)*nn.Sigmoid()(pred), sub_y).mean()
    ...
    os.makedirs("./ctcvr", exist_ok=True)
    torch.save(model.state_dict(), f"./ctcvr/data={dataset_name}_loss={loss_type}_wd={'%.0e'%weight_decay}_lr={'%.0e'%lr}_seed={random_seed}.pt")
