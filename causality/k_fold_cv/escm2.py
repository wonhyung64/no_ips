...
for cv_num, (train_idx, test_idx) in enumerate(kf.split(x_train)):
    ...
    wandb.run.name = f"cv_escm2_{loss_type}_causality_{expt_num}"
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for idx in range(total_batch):
            ...
            ctcvr_loss = nn.functional.binary_cross_entropy(ctcvr, sub_y) * beta
            ...
            rec_loss = nn.functional.binary_cross_entropy(
                nn.Sigmoid()(pred), sub_y, weight=inv_prop, reduction="none")
            rec_loss = torch.mean(rec_loss * sub_t)
            ctr_loss = nn.functional.binary_cross_entropy(nn.Sigmoid()(ctr), 1-sub_t) * alpha
            ctcvr_loss = nn.functional.binary_cross_entropy(ctcvr, sub_y) * beta
            total_loss = rec_loss + ctr_loss + ctcvr_loss
