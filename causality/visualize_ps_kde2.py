...
#%%
for dataset_name in ["original", "personalized"]:break
    ...
    labels_list = [
        ["IPS", "Multi-IPS", "ESCM2-IPS", "True"],
        ["IPS", "Multi-IPS", "ESCM2-IPS", "True"],
        ]
    density_list = []

    # for ps_model_name in ps_model_name_list:
    ps_pred_multi = np.load(f"{data_dir}/pred_t_multi_ips_plus_{dataset_name[:3]}_seed{random_seed}.npy")
    ps_pred_escm2 = np.load(f"{data_dir}/pred_t_escm2_ips_plus_{dataset_name[:3]}_seed{random_seed}.npy")
    ps_true = np.load(f"{data_dir}/true_propensity_{dataset_name[:3]}.npy")
    data_arr = np.array([ps_pred_multi, ps_pred_escm2, ps_true])
