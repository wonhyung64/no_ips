...
plt.scatter(np.arange(len(x_test_tensor)),ps_pred_plus, color="blue")
plt.scatter(np.arange(len(x_test_tensor)),ps_true, color="red")

plt.figure(figsize=(6,6))
plt.scatter(ps_true)
plt.scatter(ps_true, ps_pred_plus)

#%%
plt.figure(figsize=(6, 6))
plt.scatter(ps_true, ps_pred_plus, alpha=0.5, edgecolor='k')
plt.plot([0, 1], [0, 1], 'r--', label='Perfect prediction (y = x)')
plt.xlabel('True Propensity Score')
plt.ylabel('Predicted Propensity Score')
plt.title('True vs. Predicted Propensity Scores')
plt.legend()
plt.grid(True)
plt.show()
plt.hist(ps_true)
...
#%%
residuals = ps_pred_plus - ps_true
plt.figure(figsize=(6, 4))
plt.scatter(ps_true, residuals, alpha=0.5, edgecolor='k')
plt.axhline(0, color='r', linestyle='--')
plt.xlabel('True Propensity Score')
plt.ylabel('Prediction Error (Predicted - True)')
plt.title('Residual Plot of Propensity Score Predictions')
plt.grid(True)
plt.show()

#%%
plt.figure(figsize=(8, 4))
plt.plot(range(len(ps_true)), ps_true, label='True Propensity', alpha=0.7)
plt.plot(range(len(ps_pred_plus)), ps_pred_plus, label='Predicted Propensity', alpha=0.7)
plt.xlabel('User-Item Pair Index')
plt.ylabel('Propensity Score')
plt.title('True and Predicted Propensity per User-Item Pair')
plt.legend()
plt.grid(True)
plt.show()
